package com.hai.commons.config.xml;
...
public class XmlFileFactory {
        ...
        for (Field field : fields) {
            ...
            if (isBaseDataType(cls)) {// 如果是基础数据类型
                ...
                if (isBaseDataType(componentType)) {
                    ...
                    List list = (List) toLoadObject(config, key, componentType);
                    if (list.size() > 0) {
                ...
                if (fc instanceof ParameterizedType) {
                    ...
                    if (isBaseDataType(genericClazz)) {
                        ...
                        configValue = toLoadObject(config, key, genericClazz);
                    }
                }
            } else {// 其他则认为是对象
    ...
    private static Object toLoadObject(XMLConfiguration config, String key, Class<?> genericClazz) throws InstantiationException, IllegalAccessException {
        List list = new ArrayList();
        int i = 0;
        while (true) {
            Object loadObj = loadObj(config, key + "." + toLowerFirst(genericClazz.getSimpleName()) + "(" + i + ").", genericClazz);
            if (loadObj == null) {
                break;
            }
            list.add(loadObj);
    ...
    public static void main(String[] args) {
        Config config = XmlFileFactory.loadFile("xml/commons.config.test.xml", Config.class);
