...
public class MyCrawlerController {
    ...
    public void test() {
        ...
        //设置请求的网页的深度设置为两层.默认值-1 无限深度
        config.setMaxDepthOfCrawling(2);

        //设置爬取的最大网页数.这里设置1000,最多爬取1000次; 默认值是-1，表示无限制
        config.setMaxPagesToFetch(1000);

        //是否爬取二进制文件，比如图片，PDF文档，视频之类的东西 这里设置false 不爬取; 默认值true，爬取
        config.setIncludeBinaryContentInCrawling(false);

        //这个配置假如设置成true，当一个爬虫突然终止或者奔溃，我们可以恢复；默认配置是false；推荐用默认配置，假如设置成true，性能会大打折扣；
        config.setResumableCrawling(false);

        //这里可以设置代理
        /*config.setProxyHost("proxyserver.example.com");  // 代理地址
        config.setProxyPort(8080); // 代理端口如果使用代理，也可以设置身份认证  用户名和密码
